<template>
  <div class="site">
    ...
    <template v-if="!loading">
      <div class="filters">
        <input type="text" v-model="filter_search" placeholder="Recherchez un pokémon...">
      ...
      <div class="grid pokemons">
        <div class="animate__animated animate__shakeX" v-if="noResult">pas de résultat</div>
        <router-link
        ...
        >
          ...
          <img :src="hoverImg == index ? getSpriteShiny(pokemon) : getSprite(pokemon)" :alt="getName(pokemon)" class="pokemon-img">
          <div class="pokemon-types">
            <span class="pokemon-type" :class="type" v-for="type in pokemon.types" :key="type"></span>
          </div>
        </router-link>
      </div>
    </template>
...
export default {
  ...
  methods: {
        ...
        .finally(() => {
          axios
            ...
            .finally(() => {
              ...
              this.arrayPokemons.find(p => p.id == pokemon.id).sprite = pokemon?.sprites.front_default
              this.arrayPokemons.find(p => p.id == pokemon.id).sprite_shiny = pokemon?.sprites.front_shiny
              let typesArr = []
    ...
    filterName(pkm, val){
      return val == '' ? true : this.getName(pkm).toLowerCase().includes(val.toLowerCase()) || pkm.id == val
    },
    filterType(pkm, val){
    ...
    filter(){
      ...
      this.$timer = setTimeout(() => {
        this.filteredPkms = _.orderBy(this.arrayPokemons.filter(pkm => this.filterName(pkm, this.filter_search) && this.filterType(pkm, this.filter_type)), 'id')
