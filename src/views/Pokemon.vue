<template>
  <div class="modal d-flex">
    ...
    <div v-if="!loading" class="this-pokemon">
      <div class="col">
        <button class="back" @click.prevent="$emit('closeModal')">X</button>
        <h1>{{namePkm}}</h1>
        <p class="text-center">{{descPkm}}</p>
        <p class="types text-center">
          <span class="type" :class="type.en" v-for="type in typesPkm" :key="type.en">{{type.fr}}</span>
        </p>
      </div>
      <div class="col">
    ...
    async getInfo(){
      ...
      await axios
        ...
        .finally(() => {
          ...
          this.descPkm = infoPkm?.flavor_text_entries.find(desc => desc.language.name == 'fr' && desc.version.name == 'lets-go-pikachu').flavor_text
        })
    },
    getSprite(){
